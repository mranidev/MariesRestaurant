...
class Square extends BasePaymentGateway
{
    ...
    {
        // The newer square/square SDK makes SquareClientBuilder's constructor
        // private (factory pattern) — resolve() from the container therefore
        // fails with "Target [SquareClientBuilder] is not instantiable". Use
        // the SDK's own init() factory instead.
    ...
    protected function getPaymentFormFields($order, $data = []): array
    {
        ...
        $fields = [
            ...
            // Sandbox merchants are locked to USD; charge USD in test mode so
            // the flow can be validated end-to-end. Production locations use
            // the store currency (GBP).
            'currency' => $this->isTestMode() ? 'USD' : currency()->getUserCurrency(),
